...
public final class AsyncBidirectionalStreamingServerHandler<
...
>: GRPCServerHandlerProtocol {
  ...
  @usableFromInline
  internal let observer: (GRPCAsyncStream<Request>, AsyncResponseStreamWriter<Response>, AsyncServerCallContext) async throws -> Void
  ...
  internal enum State {
    ...
    case idle
    // Headers have been received, a context and request stream have been
    // created and passed to the user function. The handler here pokes things
    // into the request stream that was passed to the user function.
    //
    // TODO: put the context in for now and intend to remove it.
    case observing((StreamEvent<Request>) -> Void, _StreamingResponseCallContext<Request, Response>)
    // The observer has completed.
    case completed
  }
  ...
  public init(
    ...
    observer: @escaping (GRPCAsyncStream<Request>, AsyncResponseStreamWriter<Response>, AsyncServerCallContext) async throws -> Void
  ) {
  ...
  internal func receiveInterceptedMetadata(_ headers: HPACKHeaders) {
    ...
    case .idle:
      ...
      context.statusPromise.completeWithAsync {
        // We pass the existing `_StreamingResponseCallContext` in (because the
        // user is viewing it through the `AsyncServerCallContext` protocol
        // anyway).
        try await self.observer(requestStream, responseStreamWriter, context)
        return .ok  // the status promise completion handler will handle any errors thrown.
      }
  ...
  internal func handleError(_ error: Error, thrownFromHandler isHandlerError: Bool = false) {
    ...
    case let .observing(_, context):
      // TODO: The non-async-await handler did not do any cleanup here because
      // there was no promise to fail apparently. The observer used to just be a
      // closure that responded to `StreamEvent<Request>`. Now we have spun up a
      // task that is consuming responses from an AsyncSequence<Request>, we
      // should probably try and cancel that task, or terminate the stream, or
      // both.
      //
      // To cancel the task we will need to:
      // 1. Update `NIO.EventLoopPromise.completeWithAsync(_:)` to return a Task.
      // 2. Stash this in the associated data of the State.observing enum
      // value when we invoke the user function.
      // 3. Pull it out of the `State.observing` enum value here and cancel it.
...
extension AsyncBidirectionalStreamingServerHandler {
