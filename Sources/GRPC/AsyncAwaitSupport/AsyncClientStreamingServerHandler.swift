...
public final class AsyncClientStreamingServerHandler<
...
>: GRPCServerHandlerProtocol {
  ...
  @usableFromInline
  internal let observer: (GRPCAsyncStream<Request>, AsyncServerCallContext) async throws -> Response
  ...
  public init(
    ...
    observer: @escaping (GRPCAsyncStream<Request>, AsyncServerCallContext) async throws -> Response
  ) {
  ...
  internal func receiveInterceptedMetadata(_ headers: HPACKHeaders) {
    ...
    case .idle:
      ...
      // Register callbacks on the response future.
      context.responsePromise.futureResult.whenComplete(self.userFunctionCompletedWithResult(_:))
      context.responsePromise.completeWithAsync {
        // We pass the existing `UnaryResponseCallContext` in (because the
        // user is viewing it through the `AsyncServerCallContext` protocol
        // anyway).
        return try await self.observer(requestStream, context)
      }
